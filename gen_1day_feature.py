...
def prepare_data(subjects,data_dir ):
    ...
    for (s, subject) in enumerate(subjects):
        ...
        print ('to:', end_dt)
        print('time period:', (end_dt-start_dt))
        time_slices = []
        ...
        for time_slice in time_slices:
            ...
            #every day is represented by a feature vector
            feature = stat_day(time_slice,raw_data_dict)
            features.append(feature)

        features = np.array(features)

        result[subject] = np.array(features)
        print()
...
if __name__ == '__main__':
    ...
    ### features (%percentage):
    ### act_onfoot, act_still, act_invehicle, act_tilting, calling_duration, screen_on, light_off
